...
3.中国基本面 + 美国涨跌:
    Hyper parameter:
        ...
        random_seed: 273
    Result:
        val mse: 167.3267245173879
        test mse: 138.0333304072597
...
weights_path = "pkl/LSTM/"  # Model save path
...
test, val = train_test_split(others, test_size=0.5, random_state=777)
...
for input_val, label_val in zip(inputs_val, labels_val):
    ...
    with torch.set_grad_enabled(False):
        ...
        for i in input_val:
            i = i.to(device)
            out, hidden, cell = model(i, hidden, cell)
        li.append(['test', out.cpu().item(), label_val.cpu().item()])
        loss = criterion(out[0], label_val)
...
for input_test, label_test in zip(inputs_test, labels_test):
    ...
    with torch.set_grad_enabled(False):
        ...
        for i in input_test:
            i = i.to(device)
            out, hidden, cell = model(i, hidden, cell)
        li.append([out.cpu().item(), label_test.cpu().item()])
        loss = criterion(out[0], label_test)
        total_loss_test += loss.item()

df = pd.DataFrame(li,columns=['predict', 'label'])
df.to_csv("output/LSTM_regress_IWN.csv", index=False)
