...
model = BPNN(input_size=input_size, output_size=32, num_classes=1, classification=True)
...
for epoch in range(num_epochs):
    ...
    for input, label in zip(inputs, labels):
        ...
        preds_train.extend(out.cpu().tolist()[0])
        loss.backward()
        optimizer.step()
...
for input_val in inputs_val:
    ...
    with torch.set_grad_enabled(False):
        ...
        out = model(input_val)
        preds_val.append(out[0].cpu().tolist()[0])
df = pd.DataFrame({'predict': preds_val, 'label': labels_val})
df.to_csv('./output/BP_regress_IWN.csv', index=False)