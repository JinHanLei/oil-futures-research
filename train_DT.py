...
y_hat_C = DTC.predict(inputs_test_C)

df = pd.DataFrame({'predict': y_hat_C, 'label': labels_test_C})
df.to_csv("output/DT_IWN.csv", index=False)
...
y_hat_R_D = DTR.predict(inputs_test_D)

df = pd.DataFrame({'predict': y_hat_R_D, 'label': labels_test_D})
df.to_csv("output/DT_regress_IWN.csv", index=False)