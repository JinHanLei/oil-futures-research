...
3.中国基本面 + 美国涨跌:
    Hyper parameter:
        ...
        random_seed: 273
    Result:
        val mse: 0.01544560899900828
        test mse: 0.016018793852708768
"""
...
weights_path = "pkl/RGRU/"  # Model save path
...
# 中国基本面 + 美国涨跌
dataset = od.transform_zh_with_en_price()

# Split the data set
train, others = train_test_split(dataset, test_size=0.2)
test, val = train_test_split(others, test_size=0.5)
...
for input_test, media_test, label_test in zip(inputs_test, medias_test, labels_test):
    ...
    with torch.set_grad_enabled(False):
        ...
        for i, m in zip(input_test, media_test):
            ...
            m = m.to(device)
            out, hidden = model(i, m, hidden)
        output_result.append([out.cpu().tolist()[0][0], label_test.cpu().tolist()[0]])
...
df = pd.DataFrame(output_result, columns=['predict', 'label'])
df.to_csv('./output/RGRU_regress_IWN.csv', index=False)
