...
class OriginalData:
    ...
    def transform_zh_with_en_price(self):
        """
        Construct a zh_with_en_label dataset
        """
        zh_data_df = pd.read_excel(self.oil_data_path, sheet_name=[0, 1, 2])[0]
    ...
    def min_max_data_dict_regress(self, data_dict):
        ...
        X_train_minmax = min_max_scaler.fit_transform(data_dict["base"])
        # data_dict.update({"base": X_train_minmax})
        min_max_data_dict = {"base": X_train_minmax, "emotion": data_dict["emotion"]}
        data_list = [[a, b] for a, b in zip(data_dict["base"], data_dict["emotion"])]
        min_max_data_dict = [[a, b] for a, b in zip(min_max_data_dict["base"], min_max_data_dict["emotion"])]
        data_list_with_label = []
        for i in range(len(min_max_data_dict) - (self.days_iter + 1)):
            base_temp = []
            emotion_temp = []
            for j in range(self.days_iter):
                base_temp.append(torch.Tensor([min_max_data_dict[i + j][0]]))
                emotion_temp.append(torch.Tensor([min_max_data_dict[i + j][1]]))
